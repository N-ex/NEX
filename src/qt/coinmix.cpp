...
class TxViewDelegate2 : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate2() : QAbstractItemDelegate(), unit(BitcoinUnits::NEX)
    {
    }
...
void Coinmix::updateDarksendProgress()
{
    ...
    QString strAnonymizeNEXAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeNEXAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeNEXAmount = strAnonymizeNEXAmount.remove(strAnonymizeNEXAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeNEXAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeNEXAmount * COIN) nMaxToAnonymize = nAnonymizeNEXAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeNEXAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeNEXAmount));
        strAnonymizeNEXAmount = strAnonymizeNEXAmount.remove(strAnonymizeNEXAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeNEXAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
