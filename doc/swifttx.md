...
This mechanism has been integrated into Bitcore-Node-Nex which allows for notification to be broadcast through Insight API in one of two ways:
...
By default, the Nex Core daemon will launch using the following constant:

```
static const int DEFAULT_SWIFTTX_DEPTH = 5;
```

This value can be overridden by passing the following argument to the Nex Core daemon:
